...
public class JPushClientUtil extends BaseController {
	 ...
	 *            用户id（uuid）
	 */
	public static void pushMessage(String msgTitle, String msgContent,
			String userid) {
		String result = "";
		init();
		...
		if (null != msgResult) {
			...
			if (msgResult.getErrcode() == ErrorCodeEnum.NOERROR.value()) {
				result = String.format("发送成功， sendNo= %s,messageId= %s",
						msgResult.getSendno(), msgResult.getMsg_id());
				System.out.println(result);
			} else {
				result = "发送失败， 错误代码=" + msgResult.getErrcode() + ", 错误消息="
						+ msgResult.getErrmsg();
				System.out.println(result);
			}
		} else {
