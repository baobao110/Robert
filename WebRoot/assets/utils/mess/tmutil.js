...
function saveMess(obj) {
	...
	if($("#tmid").val().trim().length>1){
		messobj._id=$("#tmid").val();
	}else{
		delete messobj._id;
	}
	...
	$.ajax({
		type : 'post',
		url : getRootPath() + "/message/saveTM.do",
		data : {
...
function saveMessData() {
	...
	if (kcid != null && kcid.trim() != "") {

		messobj.kcid = kcid;
		messobj.kcname= $("#kcid").find("option:selected").text();
		
	} else {
		layer.tips('课程不能为空', '#kcid', {
			tips : [ 1, '#3595CC' ],
			time : 2000
		});
		scrollToLocation($("#kcid"));
	...
	if (question != null && question.trim() != "") {
		...
		messobj.question = question;
	
		
	} else {
		layer.tips('题目问题不能为空', '#question', {
			tips : [ 1, '#3595CC' ],
			time : 2000
		});
		scrollToLocation($("#question"));
	...
	if (chooseA != null && chooseA.trim() != "") {

		messobj.chooseA = chooseA;
	
		
	} else {
		layer.tips('题目选项A不能为空', '#chooseA', {
			tips : [ 1, '#3595CC' ],
			time : 2000
		});
		scrollToLocation($("#chooseA"));
	...
	if (chooseB != null && chooseB.trim() != "") {

		messobj.chooseB = chooseB;
	
		
	} else {
		layer.tips('题目选项B不能为空', '#chooseB', {
			tips : [ 1, '#3595CC' ],
			time : 2000
		});
		scrollToLocation($("#chooseB"));

		return false;
	}
	
	var chooseC = $("#chooseC").val();
	if (chooseC != null && chooseC.trim() != "") {

		messobj.chooseC = chooseC;
	
		
	} else {
		layer.tips('题目选项C不能为空', '#chooseC', {
			tips : [ 1, '#3595CC' ],
			time : 2000
		});
		scrollToLocation($("#chooseC"));

		return false;
	}
	var chooseD = $("#chooseD").val();
	if (chooseD != null && chooseD.trim() != "") {

		messobj.chooseD = chooseD;
	
		
	} else {
		layer.tips('题目选项D不能为空', '#chooseD', {
			tips : [ 1, '#3595CC' ],
			time : 2000
		});
		scrollToLocation($("#chooseD"));
	...
	if (answer != null && answer.trim() != "") {
		...
		messobj.answer = answer;
	
		
	} else {
		layer.tips('题目选项D不能为空', '#answer', {
			tips : [ 1, '#3595CC' ],
			time : 2000
		});
		scrollToLocation($("#answer"));
	...
	if (info != null && info.trim() != "") {
		...
		layer.tips('题目解析不能为空', '#info', {
			tips : [ 1, '#3595CC' ],
			time : 2000
		});
		scrollToLocation($("#info"));

		return false;
	}
	
	var myimg = $("#myimg").attr("data");
