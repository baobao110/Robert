...
function showMess() {
	...
	if (messobj.title) {
		...
		delete messobj.title;
	}
	if (messobj.orderby) {
		$("#orderby").val(messobj.orderby);
		delete messobj.orderby;
	}
	//内容
	if (messobj.pptx) {
		pptx=messobj.pptx;
	...
	if(messobj.uilist&&messobj.uilist.length>0){
...
function saveMess(obj) {
	...
	$.ajax({
		type : 'post',
		url : getRootPath() + "/message/saveMerMessage.do",
		data : {
...
function saveMessData() {
	...
	if ($(".onemodal").length>0) {
		$(".onemodal").each(function(){
			...
			one.video=null;
			if($(this).find(".myvideo")!= null){
				one.video = $(this).find(".myvideo").attr("srcurl");
			}
			
			one.info=$(this).find(".content").val();
		
			
			if(one.video==null&&one.img==null){
				uilist=[];
				return false;
			}
...
function createOneModalHtml(data) {
	...
	html+='<div class="layui-form-item"><label class="layui-form-label">讲解视频</label>';
	html+='<div class="layui-input-block">';
	if(myvideo != null){
		html+='<span><video name="myvideo" class="myvideo" srcurl="'+myvideo+'" src="'+getRootPath()+myvideo+'" controls="controls" width="500" height="300"></video></span>';
	}else{
		html+='<span></span>';
	}
	
	html+='<button class="layui-btn layui-btn-primary layui-btn-sm ump4"  onclick="uploadmy_video(this);">上传视频文件</button></div><div>';

	
	
	
	html+='<div class="layui-form-item"><label class="layui-form-label">讲解音频</label>';
	html+='<div class="layui-input-block">';
	if(audio != null){
	...
	/*html+='<button class="layui-btn layui-btn-primary layui-btn-sm ump3"  onclick="add_upload_show(this);">录音并上传</button>';*/
	html+='<button class="layui-btn layui-btn-primary layui-btn-sm"  onclick="uploadmy_show(this);">上传音频文件</button></div><div>';

	
	
	html += '<div class="layui-form-item">';
	html += '<label class="layui-form-label">讲解说明</label>';
	html += '<div class="layui-input-block">';
